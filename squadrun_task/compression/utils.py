...
def map_to_original(image_list_original,image_list_compressed):
    try:
        ...
        ws.write(0, 1, 'Original Size(kb)')
        ws.write(0,2,'Compressed Image  Url')
        ws.write(0, 3, 'Compressed Size(kb)')
        ws.write(0,4,'Extraction type')
        for i in range(0,len(image_list_original)):
            ws.write(i+1,0,image_list_original[i].get('url'))
            ws.write(i + 1, 1, image_list_original[i].get('size'))
            ws.write(i + 1, 2, image_list_compressed[i].get('url'))
            ws.write(i + 1, 3, image_list_compressed[i].get('size'))
            ws.write(i + 1, 4, image_list_original[i].get('message'))
        ws.col(0).width = 256 * max([len(image.get('url')) for image in image_list_original])
        ws.col(1).width = 256 * max([len(str(image.get('size'))) for image in image_list_original])
        ws.col(2).width = 256 * max([len(image.get('url')) for image in image_list_compressed])
        ws.col(3).width = 256 * max([len(str(image.get('size'))) for image in image_list_compressed])
        ws.col(4).width = 256 * max([len(image.get('message')) for image in image_list_original])
        wb.save('mapping.xls')
